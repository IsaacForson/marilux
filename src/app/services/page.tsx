    ...
    <>
          ...
          <div className="flex flex-wrap gap-2">
            {SERVICE_CATEGORIES.map((c) => (
              <Link
                key={c.slug}
                href={'#' + c.slug}
                className="rounded-full border border-white/[0.09] px-4 py-2.5 font-sans text-2xs uppercase tracking-luxe text-ivory/50 transition-all duration-500 ease-luxe hover:border-champagne/50 hover:text-champagne"
              >
                {c.name}
              </Link>
            ))}
      ...
      {SERVICE_CATEGORIES.map((category, ci) => (
        <section
          key={category.slug}
          id={category.slug}
          className="relative scroll-mt-28 border-t border-white/[0.07] py-20 sm:py-28"
                      ...
                      <Reveal y={16} delay={Math.min(i, 6) * 0.03}>
                        <div className="group flex flex-col gap-3 py-6 sm:flex-row sm:items-start sm:justify-between sm:gap-8">
                          ...
                          <div className="flex shrink-0 items-center gap-4 sm:flex-col sm:items-end sm:gap-3">
                            <span className="whitespace-nowrap font-display text-xl text-champagne">
                              {formatPrice(s)}
                            </span>
                            <Link
                              href={
                              ...
                              className="whitespace-nowrap rounded-full border border-white/12 px-4 py-2 font-sans text-2xs uppercase tracking-luxe text-ivory/60 transition-all duration-500 ease-luxe hover:border-champagne/60 hover:bg-champagne hover:text-ink"
                            >
                              Book
                            </Link>
                          </div>
