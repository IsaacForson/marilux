...
export default function AboutPage() {
  return (
    <>
        ...
        <div className="grid gap-12 lg:grid-cols-[1.05fr,0.95fr] lg:gap-20">
          ...
          <div className="pt-10 lg:pt-0">
            ...
            <div className="mt-12 space-y-8">
              <Reveal y={22}>
                <div className="flex gap-5 border-t border-line pt-8">
                  <Eye
      ...
      {/* Story timeline */}
      <section
        className="relative border-y border-line py-24 sm:py-32"
          ...
          <ol className="mt-16 grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
            {CHAPTERS.map((c, i) => (
              <Reveal key={c.year} y={28} delay={i * 0.08}>
                <li className="border-t border-accent/25 pt-7">
                  <p className="font-display text-3xl font-light text-accent">{c.year}</p>
        ...
        <div className="mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {SPECIALISTS.map((s, i) => (
            <Reveal key={s.slug} y={34} delay={(i % 3) * 0.07}>
              <article className="group h-full overflow-hidden rounded-[1.75rem] border border-line transition-colors duration-700 hover:border-line-3">
                <Plate
                  alt={'A portrait of ' + s.name + ', ' + s.role}
                  theme="portrait"
                  index={s.photoIndex}
                  ratio="aspect-[4/5]"
                ...
                <div className="p-7">
                  ...
                  <dl className="mt-6 flex items-center justify-between border-t border-line pt-4 font-sans text-2xs uppercase tracking-luxe">
                    <div className="flex gap-2">
                      <dt className="text-ivory/30">Signature</dt>
                      <dd className="text-accent/80">{s.signature}</dd>
                    </div>
                    <div className="flex gap-2">
      ...
      {/* Numbers */}
      <section className="relative border-y border-line py-20" aria-label="By the numbers">
        <div className="shell">
      ...
      <section className="shell py-24 sm:py-32" aria-labelledby="environment-title">
        <div className="grid gap-12 lg:grid-cols-[0.95fr,1.05fr] lg:gap-20">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Reveal y={36}>
              <Plate alt="The reception at Marilux" theme="studio" index={1} ratio="aspect-[3/4]" />
            </Reveal>
            <Reveal y={36} delay={0.1} className="pt-10">
              <Plate alt="A private treatment suite" theme="spa" index={3} ratio="aspect-[3/4]" />
            </Reveal>
          </div>
        </div>
      </section>
