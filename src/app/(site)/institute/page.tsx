...
/** One cover per course, in curriculum order. */
const INSTITUTE_THEMES: ImageTheme[] = [
  'brows',
  'lashes',
  'nails',
  'makeup',
...
export default function InstitutePage() {
  ...
  return (
    <>
        ...
        <div className="mt-16 grid gap-5 sm:grid-cols-2">
          {PILLARS.map((p, i) => (
            <Reveal key={p.title} y={30} delay={i * 0.07}>
              <article className="group h-full rounded-[1.75rem] border border-line p-8 transition-colors duration-700 hover:border-accent/30">
                <span className="grid h-12 w-12 place-items-center rounded-full border border-accent/25 text-accent transition-colors duration-500 group-hover:border-accent/60">
      ...
      <section
        id="courses"
        className="relative scroll-mt-28 border-y border-line py-20 sm:py-28"
          ...
          <div className="mt-16 grid gap-5 lg:grid-cols-2">
            {courses.map((course, i) => (
              <Reveal key={course.slug} y={34} delay={(i % 2) * 0.07}>
                <article className="group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-line transition-colors duration-700 hover:border-accent/30">
                  <Plate
                    alt={course.name}
                    theme={INSTITUTE_THEMES[i % INSTITUTE_THEMES.length]}
                    index={i}
                    ratio="aspect-[16/9]"
                  ...
                  <div className="flex flex-1 flex-col p-7 sm:p-8">
                    ...
                    <ul className="mt-6 flex flex-wrap gap-2">
                      <li className="rounded-full border border-line-2 px-3 py-1.5 font-sans text-2xs uppercase tracking-luxe text-ivory/45">
                        {formatDuration(course.duration)}
                      </li>
                      {course.tags?.map((t) => (
                        <li
                          key={t}
                          className="rounded-full border border-accent/25 px-3 py-1.5 font-sans text-2xs uppercase tracking-luxe text-accent/85"
                        >
                          {t}
                        </li>
                      ))}
      ...
      {graduate && (
        <section className="shell py-20 sm:py-28" aria-label="Graduate story">
          <Reveal y={34}>
            <figure className="grid gap-10 rounded-[2rem] border border-line p-8 sm:p-12 lg:grid-cols-[0.75fr,1.25fr] lg:items-center lg:gap-16">
              <Plate
                alt={'A portrait of ' + graduate.name}
                theme="portrait"
                index={4}
                ratio="aspect-square"
              ...
              <div>
                ...
                <figcaption className="mt-7 font-sans text-2xs uppercase tracking-luxe">
                  <span className="text-accent">{graduate.name}</span>
