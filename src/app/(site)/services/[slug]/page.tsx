...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      {/* The ritual */}
      <section
        className="relative border-y border-line py-20"
                ...
                <div className="flex flex-col gap-4 py-7 lg:flex-row lg:items-start lg:justify-between lg:gap-10">
                  ...
                  <div className="flex items-end justify-between gap-6 lg:flex-col lg:items-end lg:gap-3">
                    <div className="text-right">
                      <p className="whitespace-nowrap font-display text-2xl text-accent">
                        {formatPrice(s)}
                      </p>
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {specialists.map((s, i) => (
              <Reveal key={s.slug} y={30} delay={i * 0.07}>
                <article className="h-full rounded-[1.75rem] border border-line p-6">
                  <Plate
                  ...
                  <p className="mt-5 border-t border-line pt-4 font-sans text-2xs uppercase tracking-luxe text-ivory/35">
      ...
      {/* Stories */}
      {stories.length > 0 && (
        <section className="shell border-t border-line py-20" aria-label="Client stories">
          <div className="grid gap-6 md:grid-cols-2">
            {stories.map((t, i) => (
              <Reveal key={t.id} y={26} delay={i * 0.08}>
                <figure className="h-full rounded-[1.75rem] border border-line p-7 sm:p-9">
      ...
      {/* Onward */}
      <section className="shell border-t border-line py-20" aria-labelledby="next-title">
          ...
          {others.map((c, i) => (
            <Reveal key={c.slug} y={24} delay={i * 0.06}>
              <Link
                ...
                className="group flex h-full items-center justify-between gap-4 rounded-2xl border border-line p-6 transition-colors duration-500 hover:border-line-3"
              >
                <span>
                  <span
                ...
                <ArrowUpRight
                  className="h-4 w-4 shrink-0 text-ivory/30 transition-all duration-500 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-accent"
                  strokeWidth={1.4}
                  aria-hidden="true"
                />
              </Link>
