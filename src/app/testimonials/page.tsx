    ...
    <>
          ...
          <div className="glass grid gap-8 rounded-[1.75rem] p-8 sm:grid-cols-2 lg:grid-cols-4 sm:p-10">
            <div>
              <p className="flex items-baseline gap-2">
                <span className="font-display text-5xl font-light text-champagne">4.9</span>
        ...
        <div className="mt-16 space-y-6">
          {STORIES.map((t, i) => (
            <Reveal key={t.id} y={34} delay={(i % 2) * 0.07}>
              <article
                className={
                  'grid gap-8 rounded-[1.75rem] border border-white/[0.08] p-7 sm:p-10 lg:grid-cols-[0.8fr,1.2fr] lg:items-center lg:gap-12 ' +
              ...
              >
                ...
                <div className={i % 2 ? 'lg:[direction:ltr]' : ''}>
                  <Quote
                    className="h-7 w-7 text-champagne/40"
                    strokeWidth={1.2}
                    aria-hidden="true"
                  />
                  <div
        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {TESTIMONIALS.map((t, i) => (
            <Reveal key={t.id} y={26} delay={(i % 3) * 0.06}>
              <figure className="flex h-full flex-col rounded-2xl border border-white/[0.08] p-6 transition-colors duration-700 hover:border-champagne/30">
                <div className="flex gap-1" aria-label={t.rating + ' out of 5 stars'}>
                  {Array.from({ length: t.rating }).map((_, s) => (
                    <Star
                      key={s}
                      className="h-3 w-3 fill-champagne text-champagne"
                      aria-hidden="true"
                    />
                  ))}
                </div>
                ...
                <figcaption className="mt-6 border-t border-white/[0.07] pt-4">
                  <span className="block font-sans text-2xs uppercase tracking-luxe text-champagne">
                    {t.name}
                  </span>
      ...
      >
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {[
            { alt: 'Brow correction', caption: 'Two corrective sessions over four months.', b: 10, a: 40 },
            { alt: 'Even-tone course', caption: 'Six brightening treatments across sixteen weeks.', b: 205, a: 36 },
            { alt: 'Volume lash set', caption: 'First set, photographed the same afternoon.', b: 130, a: 44 },
          ].map((c, i) => (
